...
* **Monorepo:** npm workspaces, マルチパッケージ構造
...
1. **モノレポ初期化:**
   * ルートディレクトリでnpmワークスペースを初期化
   * ルート `package.json` に `workspaces` フィールドを追加
   * ディレクトリ構造を作成:
...
4. **Next.jsアプリ作成:**
   ...
   * ワークスペース依存関係を設定 (`@chirashi/components`, `@chirashi/content`)
...
7. **デュアルGitHub Actions:**
   ...
   * モノレポビルド用の新しいNext.jsワークフローを追加:
     - 依存関係をインストール: `npm ci`
     - ブログをビルド: `npm run build --workspace=blog`
     - 別のS3バケットにデプロイ
   * 両ワークフローが独立してデプロイ

8. **検証:**
   * ワークスペースインストールが動作: `npm install`
   * Gatsbyサイトがルートパスでアクセス可能
   * Next.jsアプリが起動: `npm run dev --workspace=blog`
...
1. **Content Layer セットアップ:**
   * Contentlayerをインストール: `npm install --save-dev contentlayer next-contentlayer --workspace=blog`
...
2. **MDXコンポーネント統合:**
   ...
   * `@chirashi/components/mdx` からコンポーネントをインポート
...
4. **ビルドとデプロイ:**
   ...
   * ビルド確認: `npm run build --workspace=blog`
...
以下の手順に従ってください:
1. npmワークスペースのセットアップ (apps/, packages/ 構造)
2. chiranoura-blogからpackages/content/へのコンテンツ移行 (Git履歴を保持)
3. TypeScript + tsupを使用した@chirashi/componentsパッケージの作成
...
3. @chirashi/componentsを使用するmdx-components.tsxをセットアップ
...
* MDXファイルで `@chirashi/components` から直接Reactコンポーネントをインポート可能
...
* マルチパッケージモノレポ管理のためnpm workspacesを使用
