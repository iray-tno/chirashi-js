...
export const Heading: React.FC<HeadingProps> = ({
  ...
  return (
    <Tag
      id={headingId}
      className={`group relative pl-6 leading-tight ${headingStyles[level]} ${className || ''}`}
      {...props}
    >
      <a
        href={`#${headingId}`}
        aria-label={`Link to ${extractText(children)}`}
        className="absolute left-0 w-6 text-center no-underline font-normal text-inherit opacity-0 transition-opacity duration-150 group-hover:opacity-100 focus:opacity-100"
      >
        #
      </a>
      {children}
